<nav class="bg-background w-[220px] lg:w-[260px] xl:w-[300px] 2xl:w-[350px]">
    <header class="m-2 lg:m-3 xl:m-4 h-full">
        <div class="flex flex-col h-full">
            <div class="logo flex">
                <x-application-logo class="block h-10 m-2.5 w-auto" />
                <div >
                    <h2 class="text-lg lg:text-xl xl:text-2xl font-bold">Staff Portal</h2>
                    <p class="text-xs lg:text-sm font-light">SOLADIA RESIDENCES</p>
                </div>
            </div>
            <div class="navigation flex flex-col p-4">
                <div class="hidden xl:block text-lg text-text-muted ml-0 tracking-wide">MAIN MENU</div>
                ...
                <x-sidebar-link :href="route('residents.index')" :active="request()->routeIs('residents.index')">
                    ...
                    <span class="hidden xl:inline">{{ __('Residents') }}</span>
                </x-sidebar-link>
                <x-sidebar-link :href="route('facility.index')" :active="request()->routeIs('facility.index')">
                    ...
                    <span class="hidden xl:inline">{{ __('Facilities') }}</span>
                </x-sidebar-link>
                <x-sidebar-link :href="route('add-ons.index')" :active="request()->routeIs('add-ons.index')">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" class="h-5 w-5 mr-2"><path d="M640-121v-120H520v-80h120v-120h80v120h120v80H720v120h-80ZM160-240v-80h283q-3 21-2.5 40t3.5 40H160Zm0-160v-80h386q-23 16-41.5 36T472-400H160Zm0-160v-80h600v80H160Zm0-160v-80h600v80H160Z"/>
                    </svg>
                    <span class="hidden xl:inline">{{ __('Add-ons') }}</span>
                </x-sidebar-link>
                <x-sidebar-link :href="route('reservation.index')" :active="request()->routeIs('reservation.index')">
                    ...
                    <span class="hidden xl:inline">{{ __('Reservations') }}</span>
                </x-sidebar-link>
                @can('admin-access')
                <x-sidebar-link :href="route('employees.index')" :active="request()->routeIs('employees.index')">
                    ...
                    <span class="hidden xl:inline">{{ __('Staff') }}</span>
                </x-sidebar-link>
                <x-sidebar-link :href="route('log.index')" :active="request()->routeIs('log.index')">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" class="h-5 w-5 mr-2">
                        <path d="M280-280h280v-80H280v80Zm0-160h400v-80H280v80Zm0-160h400v-80H280v80Zm-80 480q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h560q33 0 56.5 23.5T840-760v560q0 33-23.5 56.5T760-120H200Zm0-80h560v-560H200v560Zm0-560v560-560Z"/>
                    </svg>
                    <span class="hidden xl:inline">{{ __('Logs') }}</span>
                </x-sidebar-link>
                @endcan

                <div class="mb-3"></div>

                <div class="hidden xl:block text-lg text-text-muted ml-0 tracking-wide">ACCOUNT</div>
                    <x-sidebar-link :href="route('profile.edit')">
                        ...
                        <span class="hidden xl:inline">{{ __('Profile') }}</span>
                    </x-sidebar-link>
                    <x-sidebar-link :href="route('xml.index')">
                        ...
                        <span class="hidden xl:inline">{{ __('XML Import/Export') }}</span>
                    </x-sidebar-link>
                    <form method="POST" action="{{ route('logout') }}">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" class="h-5 w-5 mr-2">
                            ...
                            <span class="hidden xl:inline">{{ __('Log Out') }}</span>
                        </button>
                    </form>
            </div>
        </div>
