    ...
    <div x-data="{
        ...
        <div @dragover.prevent="dragging = true" @dragleave.prevent="dragging = false"
            ...
            class="relative mb-4 flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed px-6 py-8 text-center transition duration-[var(--transition-base)]">
            {{-- Hidden actual input --}}
            <input type="file" accept="{{ $accept }}" {{ $attributes }} x-ref="fileInput"
                @change="handleFile($event.target.files)" class="absolute inset-0 h-full w-full cursor-pointer opacity-0 mb-2">
            ...
            <div x-show="!fileName">
                <p class="text-sm font-medium text-[var(--color-text)]">
                    Drop your file here, or <span
                        class="text-[var(--color-primary)] underline underline-offset-2">browse</span>
                </p>
                @if ($hint)
            ...
            <div x-show="fileName" x-cloak class="flex items-center gap-2">
                ...
                {{-- Clear button --}}
                <button type="button" @click.stop="fileName = null; $refs.fileInput.value = ''"
                    class="ml-1 rounded p-0.5 text-[var(--color-text-muted)] transition duration-[var(--transition-base)] hover:bg-[var(--color-surface-alt)] hover:text-[var(--color-text)]">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
