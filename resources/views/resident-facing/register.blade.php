                ...
                <form method="POST" action="{{ route('resident.register.store') }}" class="w-full space-y-4">
                    ...
                    <div x-show="step === 1" x-ref="step1" class="space-y-4">
                        ...
                        <div>
                            ...
                            <x-text-input type="text" name="first_name" placeholder="Juan" value="{{ old('first_name') }}" class="w-full mb-5" required/>

                            <x-input-label>Middle Name</x-input-label>
                            <x-text-input type="text" name="middle_name" placeholder="Santos" value="{{ old('middle_name') }}" class="w-full mb-5" required/>

                            <x-input-label>Last Name</x-input-label>
                            <x-text-input type="text" name="last_name" placeholder="Dela Cruz" value="{{ old('last_name') }}" class="w-full mb-5" required/>

                            <x-input-label>Contact Number</x-input-label>
                            <x-text-input type="text" name="contact_num" placeholder="0911 222 3333" value="{{ old('contact_num') }}" class="w-full mb-5" required/>
                        </div>
                        ...
                        <div class="space-y-3 md:flex md:gap-3 md:space-y-0">
                            <div class="md:flex-1">
                                ...
                                <x-text-input type="number" name="block_num"  value="{{ old('block_num') }}" class="w-full" required/>
                            </div>

                            <div class="md:flex-1">
                                <x-input-label>Lot Number</x-input-label>
                                <x-text-input type="number" name="lot_num" value="{{ old('lot_num') }}" class="w-full" required/>
                            </div>

                            <div class="md:flex-1">
                                <x-input-label>Street Number</x-input-label>
                                <x-text-input type="number" name="street_num" placeholder="5" value="{{ old('street_num') }}" class="w-full" required/>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-between pt-4">
                            <div class="text-sm tracking-wide font-medium text-primary/75 text-center m-1">
                                ...
                                <a href="{{ route('resident.login') }}" class="font-medium hover:font-bold hover:text-primary/100">
                                    Login.
                                </a>
                            </div>

                            <button type="button" class="rounded bg-primary px-5 py-2.5 text-sm font-semibold text-white hover:bg-primary-hover transition"
                                @click="[...$refs.step1.querySelectorAll('input')].every(el => el.reportValidity()) && (step = 2)">
                    ...
                    <div x-show="step === 2" x-cloak class="space-y-4">
                        ...
                        <div class="space-y-3 md:flex md:gap-3 md:space-y-0">
                            <div class="md:flex-1">
                                <x-text-input type="email" name="email" placeholder="Email" value="{{ old('email') }}" class="w-full" required/>
                            </div>
                            <div class="md:flex-1">
                                <x-text-input type="password" name="password" placeholder="Password" class="w-full" required/>
                            </div>
                            <div class="md:flex-1">
                                <x-text-input type="password" name="password_confirmation" placeholder="Confirm Password" class="w-full" required/>
                            </div>
                        </div>
